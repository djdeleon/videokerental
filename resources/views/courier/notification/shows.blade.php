...
<div class="row">
    <div class="col-md-12 text-center">
        <div class="btn-group btn-group-md">
            ...
            <a class="btn btn-outline-secondary no-print ml-2 mb-4" href="/courier/notification">Back</a>
        </div>
    </div>
</div>
...
<div class="row" style="padding-top: 6rem">
    ...
    <div class="col-md-5">
        ...
        <span><strong>Videoke Package:</strong> {{ $receipt->videoke->name }}</span> 
        <br>
        <span><strong>Number of Rental:</strong> {{ $receipt->videoke->number }}</span>
        <br>
        <span><strong>Videoke Delivery Date:</strong> {{ $receipt->reserve_format() }}</span>
        <br>
        <span><strong>Videoke Return Date:</strong> {{ $receipt->reserve_return_format() }}</span>
        <br>
        <span><strong>Total Price:</strong> ₱{{ number_format($receipt->videoke->price, 2, '.', ',') }}</span>
        @endforeach
    </div>
    <div class="col-md-3">
        <h3>Payment Method</h3>
        <span><strong>Payment Name:</strong> {{ $receipt->payment->name }}</span>
    </div>
</div>
