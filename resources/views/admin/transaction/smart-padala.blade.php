...
@section('content')
<h4 class="font-weight-bold pb-3 mb-0">Transaction for Smart Padala</h4>
<div class="text-muted small mt-0 mb-4 d-block breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/admin"><i class="feather icon-home"></i></a></li>
        <li class="breadcrumb-item active">Transaction</li>
        <li class="breadcrumb-item active">Smart Padala</li>
    </ol>
</div>
...
<table class="table table-bordered" id="table1">
        ...
        <tbody>
            @foreach ($transactions as $transaction)
                @if ($transaction->payment->name == 'Smart Padala')
                ...
                @endif
            @endforeach
            @foreach ($anotherSmart as $smart)
                @if ($smart->payment->name == 'Smart Padala')
                    
                <tr>
                    <td>{{ $smart->id - 2 }}</td>
                    <td>{{ $smart->user->first_name }} {{ $smart->user->last_name }}</td>
                    <td>{{ $smart->payment->name }}</td>
                    <td>{{ $smart->videoke->price }}</td>
                </tr>
                @endif
            @endforeach
        </tbody>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#table1').DataTable( {
            "language": {
                "emptyTable": "No Notification for Smart Padala"
            }
        } );
    } );
</script>
