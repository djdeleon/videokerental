...
@section('content')
<h4 class="font-weight-bold pb-3 mb-0">Transaction for Bayad Center</h4>
<div class="text-muted small mt-0 mb-4 d-block breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/admin"><i class="feather icon-home"></i></a></li>
        <li class="breadcrumb-item active">Transaction</li>
        <li class="breadcrumb-item active">Bayad Center</li>
    </ol>
</div>
...
<table class="table table-bordered" id="table1">
        ...
        <tbody>
            @foreach ($transactions as $transaction)
                @if ($transaction->payment->name == 'Bayad Center')
                ...
                @endif
            @endforeach
            @foreach ($anotherBayad as $bayad)
                @if ($bayad->payment->name == 'Bayad Center')
                    
                <tr>
                    <td>{{ $bayad->id - 2 }}</td>
                    <td>{{ $bayad->user->first_name }} {{ $bayad->user->last_name }}</td>
                    <td>{{ $bayad->payment->name }}</td>
                    <td>{{ $bayad->videoke->price }}</td>
                </tr>
                @endif
            @endforeach
        </tbody>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#table1').DataTable( {
            "language": {
                "emptyTable": "No Transaction for Bayad Center"
            }
        } );
    } );
</script>
